/**
 * Snapshot de codigo_viaje en prod.gldlogistica.db_trade_fact_transportistas_viajes
 * post carga histórica. El pipeline viajes solo inserta claves nuevas vs este baseline.
 *
 * Uso:
 *   node scripts/bootstrapViajesBaseline.js
 *   node scripts/bootstrapViajesBaseline.js --upload
 *   CUTOVER_DATE=2026-06-09 node scripts/bootstrapViajesBaseline.js --upload
 *
 * Local: scripts/data/viajes_prd_baseline_codigo_viaje.json
 * S3 (Lambda): s3://{ETL_LOGS_BUCKET}/etl-success/viajes/prd_baseline_codigo_viaje.json
 */
...
const S3_KEY = "etl-success/viajes/prd_baseline_codigo_viaje.json";
const LOCAL_OUT = path.join(__dirname, "data", "viajes_prd_baseline_codigo_viaje.json");
const CUTOVER = process.env.CUTOVER_DATE || "2026-06-09";

function normalizeCodigoViaje(raw) {
  if (raw == null) return "";
...
async function fetchCodigos() {
  ...
  let res = await fetch(`https://${host}/api/2.0/sql/statements`, {
    ...
    body: JSON.stringify({
      ...
      statement: `SELECT DISTINCT codigo_viaje FROM ${TABLE} WHERE codigo_viaje IS NOT NULL AND TRIM(codigo_viaje) <> ''`,
      wait_timeout: "50s",
      disposition: "INLINE",
    }),
  });
  ...
  return (p.result?.data_array || []).map((r) => normalizeCodigoViaje(r[0])).filter(Boolean);
}
...
async function uploadPayload(payload) {
  ...
  try {
    ...
    const tmp = path.join(__dirname, ".viajes_baseline_upload.json");
...
async function main() {
  ...
  const keys = [...new Set(await fetchCodigos())].sort();
  const payload = {
  ...
  console.log(`Local: ${LOCAL_OUT} (${keys.length} codigo_viaje)`);
